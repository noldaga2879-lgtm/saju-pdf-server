...
function buildPillarsTable(pillars) {
  ...
  // 지지: 한자 + 한글을 한 줄에 나란히 (지장간은 별도 행으로 뺌)
  ...
  // 지장간: 지지 아래, 글자 하나씩 세로로 나열
  const hiddenCells = order.map(([, p]) => {
    if (!p || !p.hiddenStemsHanja) return `<td></td>`;
    const chars = Array.from(p.hiddenStemsHanja).map(c => `<div class="hidden-stem-char">${escapeHtml(c)}</div>`).join('');
    return `<td class="hidden-stems-cell">${chars}</td>`;
  }).join('');
...
const STYLE = `
  ...
  * { box-sizing: border-box; }
  ...
  .jiji-row td { border-top: none; border-bottom: none; padding-top: 2px; padding-bottom: 4px; }
  ...
  .hidden-stem-char { font-family: 'Noto Serif KR', serif; font-size: 12pt; color: #6b6450; line-height: 1.3; }
