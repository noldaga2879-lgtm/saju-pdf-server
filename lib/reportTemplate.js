...
function buildSinsalExplanation(pillars) {
  ...
  const items = Array.from(found)
    ...
    .join('');
  if (!items) return '';
  return `<div class="sinsal-explain" style="break-inside: avoid; page-break-inside: avoid;"><span class="sinsal-explain-title">신살 해설</span><ul>${items}</ul></div>`;
}
...
function buildCoverPage({ name, birthInfoText, ganDate, docTitle, subtitle }) {
  ...
  <div class="cover">
    ...
    <h1>${escapeHtml(docTitle || '종합 사주 해설서')}</h1>
    <div class="subtitle">${escapeHtml(subtitle || '타고난 여덟 글자로 읽는 인생의 흐름')}</div>
